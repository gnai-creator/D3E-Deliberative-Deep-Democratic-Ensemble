...
def pad_to_max_shape(X, Y):
    ...
    for x, y in zip(X, Y):
        ...
        if x.ndim == 2:
            x_pad = np.pad(x, ((0, pad_h), (0, pad_w)), mode='constant')
        elif x.ndim == 3:
            x_pad = np.pad(x, ((0, pad_h), (0, pad_w), (0, 0)), mode='constant')
        else:
...
def pad_to_30x30_top_left_single(X, pad_value=0):
    ...
    for x in X:
        if x.ndim == 2:
            ...
            x_pad = np.full((30, 30, x.shape[2]), pad_value, dtype=x.dtype)
            h, w = x.shape[:2]
            x_pad[:h, :w, :] = x
        else:
            raise ValueError("Shape inválido para pad_to_30x30_top_left_single")
        padded_X.append(x_pad)
