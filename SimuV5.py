...
class SimuV5(tf.keras.Model):
    def __init__(self, hidden_dim=32):
        ...
        self.fallback_dense = tf.keras.layers.Dense(40)
    ...
    def call(self, x, training=False):
        ...
        features_safe = self.fallback_dense(features_safe)
        flip_logits = self.flip.logits_layer(features_safe)
        
        rotation_code = self.fallback_dense(features_safe)

        def flip_op(pair):
            img, code = pair  # code tem shape [4] ou [batch, 4]
            # Pegamos o valor dominante (maior índice) para o código de flip
            code = tf.argmax(code, axis=-1)  # shape: scalar
            ...
            return tf.switch_case(
                ...
                branch_fns={
                    0: lambda: img,
                    1: lambda: tf.image.flip_left_right(img),
                    2: lambda: tf.image.flip_up_down(img),
                    3: lambda: tf.image.flip_up_down(tf.image.flip_left_right(img)),
                },
                default=lambda: img
            )
        ...
        def rotate_single(args):
            img, k_val = args
            k_val_scalar = tf.reshape(k_val, [])  # força um escalar
            return tf.image.rot90(img, k=tf.cast(k_val_scalar, tf.int32))
        print("[DEBUG] flip_logits shape:", flip_logits.shape)
        x = tf.map_fn(flip_op, (x, flip_logits), fn_output_signature=tf.float32)
