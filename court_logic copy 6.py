...
def arc_court_supreme(models, X_test, task_id=None, block_idx=None,
    ...
    y_suprema = tf.expand_dims(y_sup, axis=-1)  # usa a moda bruta das juradas

    log(f"[DEBUG] y_suprema final shape: {y_suprema.shape}")  # Deve dar (1, 30, 30, 1)
    ...
    y_antitese = inverter_classes_respeitando_valores(y_suprema, classes_validas, pad_value=pad_value)
    y_antitese = tf.expand_dims(y_antitese, axis=-1)
