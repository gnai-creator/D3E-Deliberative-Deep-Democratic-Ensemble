...
pesos = {
    ...
    "modelo_1": 1.0,
    "modelo_2": 1.0,
    "modelo_3": 1.5,  # Advogada
    "modelo_4": 2.0,  # Juiz
    "modelo_5": 3.0,  # Suprema
    "modelo_6": -2.0
 }
...
def arc_court_supreme(models, X_test, task_id=None, block_idx=None,
    ...
    # Atualiza voto da Suprema com padrão simbólico (não depende da moda inicial das juradas)
    y_sup = gerar_padrao_simbolico(X_test)
    y_sup = tf.expand_dims(y_sup, axis=-1)       # (1, 30, 30, 1)
    y_suprema = tf.tile(y_sup, [1, 1, 1, 10])     # (1, 30, 30, 10)

    log(f"[DEBUG] Suprema valores únicos: {tf.unique(tf.reshape(y_suprema, [-1]))[0].numpy()}")

    log(f"[DEBUG] y_suprema (simbolico) shape: {y_suprema.shape}")
    x_suprema = prepare_input_for_model(5, X_test)
    modelos[5].fit(x=x_suprema, y=y_suprema, epochs=epochs * 3, verbose=0)

    # Promotor treinado com antítese coerente
    x_promotor = prepare_input_for_model(6, X_test)
    y_antitese = inverter_classes_respeitando_valores(tf.squeeze(y_suprema), classes_validas, pad_value=pad_value)
    ...
    while iter_count < max_cycles:
        ...
        # Suprema (continua com base na moda das juradas após 1a rodada)
        ...
        y_suprema = tf.expand_dims(y_sup, axis=-1)
        ...
        y_antitese = inverter_classes_respeitando_valores(tf.squeeze(y_suprema), classes_validas, pad_value=pad_value)
        y_antitese = tf.expand_dims(y_antitese, axis=-1)
        y_antitese = tf.tile(y_antitese, [1, 1, 1, 10])  # (1, 30, 30, 10)
